...
export function logoFor({ variant = "complete", color, format = "png", width, background } = {}) {
  ...
  let info;
  try { info = resolveArtwork("logo", v, { color, background }); }
  catch (e) { return text(e.message); }

  const w = fmt === "png" ? Number(width) || 2000 : width ? Number(width) : undefined;
  const url = renderUrl("logo", v, fmt, { color, width: w, background });
  ...
  for (const f of ["svg", "png", "pdf"]) {
    if (f === fmt) continue;
    out.push(`+ ${f}: ${renderUrl("logo", v, f, { color, width: f === "png" ? w || 2000 : undefined, background })}`);
  }
...
export function motifFor({ name, color, format = "svg", size } = {}) {
  ...
  const url = renderUrl("motif", n, fmt, { color, width: w });
  ...
  for (const f of ["svg", "png", "pdf"]) {
    if (f === fmt) continue;
    out.push(`+ ${f}: ${renderUrl("motif", n, f, { color, width: f === "png" ? w || 1000 : undefined })}`);
  }
  if (fmt === "svg") {
    const rel = "motifs/LAIO-" + n.toUpperCase() + ".svg";
    const raw = load(rel);
    const c = color ? resolve(color) : null;
    if (raw) {
      out.push("");
