...
export function logoFor({ variant = "complete", color, format = "png", width, background } = {}) {
  ...
  if (color && !resolve(color)) {
    useColor = undefined;
    colorNote = isLed(v)
      ? `There is no ${color} LED mark in the kit. LED ships white and black only. Here is white, which is the default on dark grounds. Black for light grounds is below.`
